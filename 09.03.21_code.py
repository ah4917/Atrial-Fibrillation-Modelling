# -*- coding: utf-8 -*-
"""
Created on Tue Mar  9 12:48:59 2021
...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb 27 16:26:58 2021
...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 22 10:55:38 2021
...
def GenerateCoorArray(N,x_size,y_size):#prepares a list with the coordinates of all nodes
    CoorStore=[[random.random()*x_size,random.random()*x_size] for i in range(N)]
    return sorted(CoorStore , key=lambda k: [k[0], k[1]])
...
def PaceMaker(StateListP,FunctionListP,N,coor):#excites pacemaker cells every T time steps
    maxloc=max([i for i in range(len(coor)) if coor[i][0]<1])
...
#def PaceMaker(StateListP,FunctionListP,N):#excites pacemaker cells every T time steps
#    StateListP[2050]= FunctionListP[0][200] + 1 
#    StateListP[50]= FunctionListP[0][200] + 1                
#    return StateListP
...
def RunState(TimeSteps,N,x_size,y_size,tau,delta,epsilon,r,T):
    ...
    for i in range(TimeSteps):
        if i == 0 or i % T == 0:          
            StateListR = PaceMaker(StateListR,FunctionListR,N,Coordinates)
...
def MovieNodes(a,frame):#input is list with all the lists describing the state of nodes(StateStore)
    ...
    if frame==None:
        ...
        for i in range(len(a[0])):
            im=plt.scatter(x,y,c=a[0][i],edgecolors='r',cmap=plt.cm.binary,vmin=0,vmax=max_v)
            ims.append([im])
        plt.colorbar()
...
def Pixelation(cc,x_grid_size,y_grid_size):
    ...
    for i in range(len(cc[0])):
        ...
        grid_degree= np.zeros([int(y_grid_size),int(x_grid_size)])
        for cell in range(len(grid_container)):
            sum_c = 0
            sum_degree=0
            for node in range(len(grid_container[cell])):
                sum_c += cc[0][i][grid_container[cell][node]]
                sum_degree+=len(cc[2][grid_container[cell][node]])
            grid_sum[grid_coor[cell][1]][grid_coor[cell][0]]=sum_c
            grid_degree[grid_coor[cell][1]][grid_coor[cell][0]]=sum_degree
            timeseries[cell].append(sum_c)
        allgridvalues.append(grid_sum)
        alldegreevalues.append(grid_degree)                
    nodespc=[]#nodespercell(determining cell with max number of nodes)
    for i in range(len(timeseries)):
        nodespc.append(max(timeseries[i]))
    maxcellcolor=max(nodespc)#determining max value possible 
    #in grid_sum,required to set the color scale
    return allgridvalues,int(maxcellcolor) ,timeseries,grid_container,grid_coor,alldegreevalues
...
def MoviePixels(pixeldata,frame):
    ...
    if frame==None:
        ...
        ani=plt.imshow(Allgridvalues[frame],interpolation='none',cmap=plt.cm.binary,vmin=0,vmax=pixeldata[1],animated=True)
        plt.gca().invert_yaxis()
        fig.colorbar(ani)
    return ani
...
def focal_quality_indicator(vectors,coord,focalpoint,timestep,vcond,tau):
    ...
    return quality
#next function is same but is supposed to save the connections
def focal_quality_indicator2(vectors,coord,connections,timestep,vcond,tau):
    ...
    return quality_all
#%%changes to focal indicator so that it records each vector once
...
def miss_fire2(x,T,Tau,variation):
    ...
    for i in range(len(miss_fire_list_ratio)-1):
    ...
    if t_ident!=0:
        ...
        locn=None    
    return locn,t_ident
#following function calculates average conduction velocity of system
def condvelavg(smltn):
    #smtln is the result of the RunState function 
    xdim=smltn[8][2]
    #minloc finds coordinates ate the edge to left
    minloc=min([i for i in range(len(smltn[1])) if smltn[1][i][0]>xdim-1])
...
def generate_macro_vectors_weightedGt(data,threshold_c,threshold_t):
    ...
    return vector_store
#%%
def dot_product_averageA(runstate,x_dim,y_dim,connections,macro = False,threshold_c=0,threshold_t=0,animate=False,display=False,data=False):
    x = runstate
    tau=x[8][4]
    period=x[8][8]
...
#        connections.append(conn_t)
    ...
    if display==True:
        ...
        plt.scatter(xloct,yloct,c='k',marker='o')
        fig.colorbar(s)
        plt.gca().invert_yaxis()
...
#%%
def dot_product_averageAn(runstate,x_dim,y_dim,sigma,macro = False,threshold_c=0,threshold_t=0,animate=False,display=False,data=False):
    x = runstate
    tau=x[8][4]
    period=x[8][8]
    vectors = Resultant_Vectors(x,outv=True)
    if macro == False:
        ...
        pixels = Pixelation_noise(x,x_dim,y_dim,sigma)
        vectors_pixelated = [generate_macro_vectors_weightedGt(pixels,threshold_c,threshold_t),pixels[4]]
...
def Pixelation_noise(cc,x_grid_size,y_grid_size,sigma):
    ...
    for i in range(len(cc[0])):
        ...
        for cell in range(len(grid_container)):
            ...
            for node in range(len(grid_container[cell])):
                sum_c += cc[0][i][grid_container[cell][node]]
                sum_ci=random.gauss(sum_c,sigma)
            grid_sum_noise[grid_coor[cell][1]][grid_coor[cell][0]]= sum_ci
            timeseries[cell].append(sum_ci)
