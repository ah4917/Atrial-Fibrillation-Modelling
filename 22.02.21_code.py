...
def generate_macro_vectors_weightedM(data,threshold):
            ...
            for i in range(len(neighbours)):
                if neighbours[i]<0:
                    ...
                    if abs(neighbours[i])<abs(neighbours[l]):
                        ...
                        neighbours[l]=0
                    elif abs(neighbours[i])==abs(neighbours[l]):
                        neighbours[i]=0
                        neighbours[l]=0
    ...
    return vector_store
#%%
def generate_macro_vectors_weightedJ(data,threshold):
    #data output of pixelation
    charge_time = data[0]
    time_length = len(charge_time)
    x_dim = len(charge_time[0])
    ...
    return vector_store
#%%following is the time series method
def generate_macro_vectors_weightedG(data,threshold):
    ...
    return vector_store
#%%
def dot_product_averageA(runstate,x_dim,y_dim,macro = False,threshold = 0,animate=False,display=False):
    x = runstate
    tau=x[8][4]
    ...
    if macro == False:
        ...
        vectors_pixelated = [generate_macro_vectors_weightedG(pixels,threshold),pixels[4]]
