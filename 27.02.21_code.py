...
def generate_macro_vectors_weightedGt(data,threshold_c,threshold_t):
    ...
    return vector_store
#%%
def dot_product_averageA(runstate,x_dim,y_dim,macro = False,threshold_c=0,threshold_t=0,animate=False,display=False,data=False):
    x = runstate
    tau=x[8][4]
    period=x[8][8]
...
def dot_product_averageAn(runstate,x_dim,y_dim,sigma,macro = False,threshold_c=0,threshold_t=0,animate=False,display=False,data=False):
    ...
    if data==True:
        if tM==0:
            ...
            return np.array([xloct[0],yloct[0]])
#def dot_product_averageAn(runstate,x_dim,y_dim,sigma,macro = False,threshold_c=0,threshold_t=0,animate=False,display=False):
#    x = runstate
#    tau=x[8][4]
#    period=x[8][8]
#    vectors = Resultant_Vectors(x,outv=True)
...
#    for t in range(tM,len(x[0])-5):
#        print(t)
#        q_all=focal_quality_indicator3(vectors_pixelated[0],vectors_pixelated[1],connections,t,vcond,5)
#        q_alltot+=np.array(q_all)/(len(x[0])-5-tM)        
#        if t<10:
#            for ti in range(0,t+1):
#                q_allon[ti]+=np.array(q_all)/10
